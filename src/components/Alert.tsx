...
interface AlertProps {
  ...
  icon?: boolean
  dismissible?: boolean
  alertId?: string // Nuevo prop para identificación única
}
...
const Alert: FC<AlertProps> = ({
  ...
  const iconVar: Record<Type, React.ReactNode> = {
    default: (
      <svg
  ...
  return (
    ...
    >
      {icon && (
        <div
          className={`flex items-center justify-center self-center size-9 rounded-full ${colorClass}`}
        >
          {iconClass}
        </div>
      )}
      ...
      {dismissible && (
        <button
          ...
          className='p-1 transition duration-200 ease-in hover:bg-inherit cursor-pointer'
        >
          <svg
            xmlns='http://www.w3.org/2000/svg'
            width='24'
