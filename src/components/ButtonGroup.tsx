...
const ButtonGroup: React.FC<ButtonGroupProps> = ({
  ...
  const hoverColors = {
    default: 'hover:bg-zinc-700/10 ',
    primary: 'hover:bg-blue-500/10',
    secondary: 'hover:bg-indigo-500/20',
    success: 'hover:bg-green-500/30',
    warning: 'hover:bg-yellow-500/30',
    danger: 'hover:bg-red-500/10'
  }

  const shadowColors = {
    default: 'shadow-md shadow-neutral-100/20',
    ...
    >
      {buttons.map((button, index) => {
        return (
          <button
            key={index}
            className={`cursor-pointer inline-flex gap-x-1 items-center font-medium transition duration-300 ${groupClass}
              ${
                index === 0
                  ? roundedS[rounded]
